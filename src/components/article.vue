    ...
    <div v-if="!$common.isEmpty(article)">
      ...
      <div style="background: var(--background);">
        <div class="article-container my-animation-slide-bottom">
          ...
          <!-- 最新进展 -->
          <div v-if="false" class="process-wrap">
            <el-collapse accordion value="1">
              <el-collapse-item title="学习进度" name="1">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getArticle() {
        const article = this.$store.state.articles.find(a => a.id === parseInt(this.id));
        if (article) {
