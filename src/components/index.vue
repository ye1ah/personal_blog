      ...
      <template slot="body">
        ...
        <div class="page-container-wrap">
          <div class="page-container">
            <!-- 个人信息卡片 -->
            <div class="profile-card my-animation-slide-bottom">
              <div class="profile-header">
                <!-- <el-avatar :size="100" :src="$store.state.webInfo.avatar"></el-avatar> -->
                <h2>{{ $store.state.webInfo.title }}</h2>
              </div>
              <div class="profile-content">
                ...
                <div class="profile-section">
                  <h3>教育背景</h3>
                  <p>2022-至今 XX大学 计算机科学与技术 硕士研究生</p>
                  <p>2018-2022 XX大学 计算机科学与技术 本科</p>
                </div>
                <div class="profile-section">
                  <h3>获奖情况</h3>
                  <p>2023年 国家奖学金</p>
                  <p>2022年 优秀毕业生</p>
                  <p>2021年 全国大学生计算机设计大赛一等奖</p>
                </div>
              </div>
            </div>
  ...
  export default {
    ...
    methods: {
      async selectSort(sort) {
        ...
        this.articles = this.$store.state.articles.filter(article => article.sortId === sort.id);
        this.$nextTick(() => {
          this.indexType = 2;
      ...
      async selectArticle(articleSearch) {
        ...
        this.articles = this.$store.state.articles.filter(article => 
          article.articleTitle.toLowerCase().includes(articleSearch.toLowerCase())
        );
        this.$nextTick(() => {
          this.indexType = 2;
          $(".announcement").css("max-width", "780px");
      ...
      getSortArticles() {
        ...
        const sortArticles = {};
        this.$store.state.sortInfo.forEach(sort => {
          sortArticles[sort.id] = articles.filter(article => article.sortId === sort.id);
        });
...
<style scoped>
  ...
  .background-image-index-error {
    background: linear-gradient(45deg, var(--lightGreen), var(--themeBackground));
    width: 100vw;
    height: 50vh;
    position: fixed;
    z-index: -1;
